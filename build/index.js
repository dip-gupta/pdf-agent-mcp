...
class PDFProcessor {
    static async getPdfMetadata(pdfData) {
        try {
    ...
    static async extractTextFromPdf(pdfData, pageRange) {
        try {
            const pdfBytes = this.base64ToUint8Array(pdfData);
            const pdfDoc = await PDFDocument.load(pdfBytes);
    ...
    static async searchInPdf(pdfData, searchText, options = {}) {
        try {
    ...
    static async getPdfOutline(pdfData) {
        try {
            const pdfBytes = this.base64ToUint8Array(pdfData);
            const pdfDoc = await PDFDocument.load(pdfBytes);
    ...
    static parsePageRange(range, totalPages) {
        if (!range) {
